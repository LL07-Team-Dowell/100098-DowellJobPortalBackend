...
def update_project_details(company_id,_date):
    ...
    if response.status_code == 200:
        ...
        res = {'status':f'error-{response.status_code}','message':"No Project details with these details exists"}
        return res
    elif response.status_code == 304:
        res = {'status':f'error-{response.status_code}','message':'project details not updated'}
        return res
    else:
        res = {'status':f'error-{response.status_code}','message':'project details not updated'}
        return res
