...
class create_task(APIView):
    def post(self, request):
            ...
            if data:
                ...
                if insert_response:
                    return Response({"message":f"Task added successfully and the status is {field['status']}"},status=status.HTTP_200_OK)
                else:
...
class get_task(APIView):
    def post(self, request):
            ...
            if data :
                ...
                print(response)
                if response:
                    return Response({"message":"List of the task","response":json.loads(response)},status=status.HTTP_200_OK)
                else:
                    return Response({"message":"There is no task" , "response":json.loads(response)},status=status.HTTP_204_NO_CONTENT)
            else:
